...
const GrainyGradient = forwardRef(({ ripples = [], onTimeUpdate }, ref) => {
  ...
  useFrame((state) => {
    if (mesh.current) {
      ...
      const activeRipples = ripples.filter(r => currentTime - r.startTime < 2.0)
      const positions = []
      const times = []

      for (let i = 0; i < Math.min(activeRipples.length, 10); i++) {
        const ripple = activeRipples[i]
        positions.push(ripple.normX, ripple.normY)
